...
inputs = {
  ...
  iam_policies = {
    access_csv_and_results = {
      ...
      template_vars = {
        vars = {
          csv_bucket_arn     = "arn:aws:s3:::${dependency.output_bucket.outputs.bucket}"
          results_bucket_arn = "arn:aws:s3:::${dependency.athena_results_bucket.outputs.bucket}"
        }
      }
